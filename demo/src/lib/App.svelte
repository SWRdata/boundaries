<script>
	...
	const tileUrl = false ? `http://127.0.0.1:8080/tiles/boundaries/{z}/{x}/{y}?dt=${Date.now()}` : `https://static.datenhub.net/data/boundaries/boundaries_2025_01-01.versatiles?{z}/{x}/{y}?dt=${Date.now()}`

	let hoverCoords = $state([])

	let tooltipCoordinates = $derived(hovered ? hoverCoords : [0, 0])
	const levels = ['staat', 'land', 'kreis', 'gemeinde']

	const fills = [tokens.shades.forest.light3, tokens.shades.red.base, tokens.shades.teal.light1, tokens.shades.yellow.light1, tokens.shades.violet.light1, tokens.shades.blue.light1]
...
<figure class="container">
	...
	>
		...
		<VectorLayer
			...
			paint={{
				'fill-color': [
					...
					'04',
					tokens.shades.yellow.light1,
					'05',
					tokens.shades.violet.light1,
					'06',
					tokens.shades.blue.light1,
					'07',
					tokens.shades.forest.light1,
					'08',
					tokens.shades.plum.base,
					'09',
					tokens.shades.yellow.base,
					'10',
					tokens.shades.orange.base,
					'11',
					tokens.shades.blue.base,
					'12',
					tokens.shades.forest.base,
					'13',
					tokens.shades.apple.base,
					'14',
					tokens.shades.violet.base,
					'15',
					tokens.shades.orange.light1,
		...
		<VectorLayer
			...
			filter={['==', 'kind', filter]}
			paint={{
				'line-width': ['case', ['any', ['boolean', ['feature-state', 'hovered'], false]], 1.75, 0.75],
				'line-color': ['case', ['any', ['boolean', ['feature-state', 'hovered'], false]], 'black', tokens.shades.gray.base],
				'line-opacity': 1,
			}}
		/>
		<VectorLayer
